...
| 验证时间 | 数据库 | 结果 |
|----------|--------|------|
| 2026-07-20 | stockdb (Aliyun RDS MySQL) | ✅ 14 张表结构与 ORM 完全一致 |

验证方式：`DESCRIBE` + `SHOW INDEX` 逐表比对。实际库中 `created_at` / `trained_at` 列为 `DEFAULT_GENERATED now()`，`is_active` 映射为 `tinyint(1)`，其余列名、类型、唯一约束、索引均与 ORM 定义一致。
